...
{
   // Objetos s�o desenhados a partir do primeiro
   // entao levar pra frente do desenho � trazer pro fim da lista
   unsigned int n = (unsigned int)_objects.size() ;
...
{
      // Objetos s�o desenhados a partir do primeiro
   // entao levar pra tras do desenho � trazer pro inicio da lista
   unsigned int n = (unsigned int)_objects.size() ;
   ...
   for( unsigned int i=0 ; i<n ; i++ )
   {
      if( _objects[i] == o )
      {
         ...
         // pois a compara��o � j>0 e n�o j>=0
