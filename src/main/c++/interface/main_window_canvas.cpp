...
static double _dragging_start_y ;

static void exportToPDF( Project *p ) ;


static std::vector<Object*> selectByBoundingBox( TopoEditor* editor, double xmin, double xmax, double ymin, double ymax )
{
	...
	for( int o = 0; o < editor->numObjects(); o++ )
	{
		...
		{
			objects.push_back( editor->_objects[o] ) ;
		}
	}
   return objects;
}
...
{ 
	...
	if( _builder )
	{
		...
		if( new_obj )
		{
         ...
         AddObjectsToEndCommand* command = new AddObjectsToEndCommand( &(_project->editor()), list );
	...
	if( _manipulator )
	{
      ...
      if (retorno || _manipulator->manipulating())
      {
         if(retorno)
            repaintMainCanvas_cb(0, 0, 0 );
         return IUP_DEFAULT;
      }
	}
	...
	if( button == IUP_BUTTON1 )
	{
		...
		{
			_pressed = true;
			_dragging_start_x = xf;
			_dragging_start_y = yf;
			return IUP_DEFAULT;
		}
		...
		if( !_dragging )
		{
			...
			if( y < h && y >=0 && x < w && x >=0 )
			{
				...
				if( sel && ( (_selected_obj.size() != 1) ||  (_selected_obj.size() == 1 && _selected_obj[0] != sel) ))
				{
					_selected_obj.clear();
					if( sel )
					{
                  _selected_obj.push_back( sel );
						_manipulator = sel->manipulator() ;
					}
				}   
				else
				{
		...
		else
		{
			...
			std::vector< Object* > list = selectByBoundingBox( &(_project->editor()), min._x, max._x, min._y, max._y );
			if( list.size() > 0 )
			{
				_selected_obj = list;
				...
				{
					_manipulator = ListManipulator::instance( list );
				}
			}
			else
			{
